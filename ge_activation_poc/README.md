...
This POC covers two setup areas (Jira comes later):
- **App setup** — who the first app is for, and what it does.
- **Infrastructure** — region / data residency + encryption (CMEK vs Google-managed). *(one-way)*

Both follow the same three beats: **Discovery → Recommendation (pros/cons) → Decision saved.**
...
ge_activation_poc/
  grounding/            # local grounding playbooks (the agent's knowledge)
    app_setup.md  region.md  encryption_cmek.md
  ge_agent/
