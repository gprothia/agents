...
def _get_plan_path() -> str:
    env_path = os.getenv("PLAN_PATH")
    if env_path:
        return env_path
    local_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "plan.xlsx")
    dir_name = os.path.dirname(local_path)
    if dir_name and os.access(dir_name, os.W_OK):
        return local_path
    return "/tmp/plan.xlsx"


PLAN_PATH = _get_plan_path()
