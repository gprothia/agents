...
pkg_name = current_dir.name
project_root = str(current_dir.parent if current_dir.name in ("askhr_oauth", "askhr_oauth_sunlit", "askhr_oauth _sunkit") else current_dir)
...
def build_app() -> AdkApp:
    ...
    try:
        import importlib
        mod = importlib.import_module(f"{pkg_name}.agent")
        root_agent = mod.root_agent
    except Exception:
        try:
            from agent import root_agent  # type: ignore
        except ImportError:
            from askhr_oauth.agent import root_agent  # type: ignore
...
def main() -> None:
    ...
    project = os.getenv("GOOGLE_CLOUD_PROJECT", "sunlit-segment-396117")
    deployment_location = os.getenv("DEPLOYMENT_LOCATION", "us-central1")

    staging_bucket = os.getenv("STAGING_BUCKET", "gs://adk-sunlit-segment-396117")
