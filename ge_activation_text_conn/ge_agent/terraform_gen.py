...
def _connector_tf(dec: dict) -> str:
    """Gmail (first-party google_mail) connector.

    Uses google_discovery_engine_data_connector because the data_store resource's
    workspace_config block is not yet in the HCL provider. Actions become live after a
    Workspace admin completes the one-time OAuth consent.
    """
    v = dec.get("value", {})
    name = _slug(v.get("connector_name", "gmail"))
    entities = ", ".join(f'"{e}"' for e in v.get("entities", ["messages"]))
    actions = v.get("actions", [])
    action_hcl = ""
    if v.get("actions_enabled") and actions:
        acts = ", ".join(f'"{a}"' for a in actions)
        action_hcl = f'''
  # Actions become active after an admin completes the one-time OAuth consent.
  action_config {{
    action_params = jsonencode({{ enabled_actions = [{acts}] }})
...
resource "google_discovery_engine_data_connector" "{name}" {{
  ...
  # First-party Workspace connector. entities: {entities}
  params = jsonencode({{
    instance_uri = "workspace"      # domain-wide via Workspace customer ID
    entities     = [{entities}]
  }})

  refresh_interval = "{v.get('refresh_interval', '86400s')}"
  {action_hcl}
}}
