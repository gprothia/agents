...
async def _call_gemini_image_api(
    ...
    for i in range(get_image_generation_tenacity_attempts()):
        ...
        except (api_exceptions.ResourceExhausted, api_exceptions.ServerError) as e:
            log_message(f"In _call_gemini_image_api received a retryable error ({type(e).__name__}). Will attempt again: {e}", Severity.WARNING)
